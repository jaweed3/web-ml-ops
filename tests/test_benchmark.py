...
def test_both_onnx_formats_present(results_by_format):
    for fmt in ("onnx_fp32", "onnx_int8"):
        assert fmt in results_by_format, (
            f"Format '{fmt}' missing from benchmark report"
        )
...
def test_int8_smaller_than_fp32(results_by_format):
    ...
    assert int8_mb < fp32_mb, (
        f"INT8 ({int8_mb} MB) should be smaller than FP32 ({fp32_mb} MB)"
    )
...
def test_int8_faster_than_fp32(results_by_format):
    ...
    assert int8_lat < fp32_lat, (
        f"INT8 ({int8_lat} ms) should be faster than FP32 ({fp32_lat} ms)"
    )
