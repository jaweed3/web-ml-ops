...
def test_output_shape_landscape_input():
    blob, _ = ImagePreprocessor(imgsz=640).run(make_jpeg(480, 640))
    assert blob.shape == (1, 3, 640, 640)


def test_output_shape_portrait_input():
    blob, _ = ImagePreprocessor(imgsz=640).run(make_jpeg(1080, 720))
    assert blob.shape == (1, 3, 640, 640)
...
def test_custom_imgsz_320():
    blob, _ = ImagePreprocessor(imgsz=320).run(make_jpeg())
